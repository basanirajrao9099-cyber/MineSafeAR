...
/**
 * What the trainee sees when the drill ends: the score, the mistakes behind it, and
 * a way onward.
 *
 * Stateless and driven entirely by [outcome], which arrives as route arguments — by
 * the time this is on screen the drill's composition, its AR session and its anchors
 * are gone.
 *
 * The result has already been written to Room by [FireModuleScreen]; this screen
 * does not save anything, so backing out of it cannot lose an attempt.
 */
@Composable
fun FireModuleResultsScreen(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.fire_module_title),
        ...
        // The three numbers requirement 5 asks the module to track, in the order a
        // trainer reads them: what went wrong, then how long it took.
        ResultRow(
        ...
        Text(
            text = stringResource(
                if (outcome.passed) R.string.fire_module_results_passed_advice
                else R.string.fire_module_results_failed_advice
            ),
        ...
        // Whichever action the score points to gets the filled button. A failed
        // drill should lead back into the drill, not away from it.
        if (outcome.passed) {
...
@Composable
private fun ScoreBadge(outcome: FireDrillOutcome) {
